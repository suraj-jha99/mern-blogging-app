...
app.post('/post', uploadMiddleware.single('file'), async (req,res) => {
  //renaming file path name to new path : adding .extension [e.g .png] at the end of file name
  ...
  jwt.verify(token, secret, {}, async (err,info) => {
    ...
    const postDoc = await Post.create({
      ...
      summary,
      content,
      cover:newPath,//file name for uploaded file[inside uploads folder]
      author:info.id,
    });
    res.json(postDoc);
  });
...
//to update a post or edit a post
app.put('/post',uploadMiddleware.single('file'), async (req,res) => {
  let newPath = null;

  //if we have an update file : rename the file 
  if (req.file) {
    const {originalname,path} = req.file;
    const parts = originalname.split('.');
    const ext = parts[parts.length - 1];
  ...
  jwt.verify(token, secret, {}, async (err,info) => {
    ...
    //find the post in db using post id
    const postDoc = await Post.findById(id);
    //if post's author and cur user are the same or not
    const isAuthor = JSON.stringify(postDoc.author) === JSON.stringify(info.id);
    if (!isAuthor) {
      return res.status(400).json('you are not the author');
    }
    //update posts info in database
...
//fetching all the posts form database and sort them by latest posts order and limit posts t0 20
...
//to get a particular post document
